...
Kesi TypeScript 客户端库，为 React 应用提供完整的 API、认证、表单处理和模型管理功能。
...
- **模型管理** - 集成的状态管理和数据操作
- **Page Hooks** - 页面级状态管理和数据订阅
- **数据订阅** - WebSocket 实时数据推送和报警订阅
- **类型安全** - 完整的 TypeScript 类型定义

## 安装

```bash
npm install @kesi/client
# 或
yarn add @kesi/client
# 或
pnpm add @kesi/client
```
...
```typescript
import { useLogin, useLogout, useUser } from '@kesi/client'
...
```typescript
import { SchemaForm, setFormFields, Form, FieldArray } from '@kesi/client'
...
```typescript
import { Model, useModelList } from '@kesi/client'
