 ...
 * @param records 设备记录列表（来自 createAPI.query）
...
async function fetchLatestTags<T extends Record<string, any>>(
  ...
  try {
    ...
    const res: any = await dataApi.fetch('/latest', { method: 'POST', data: points })
...
const deviceApi = createAPI({ resource: 'core/t/hvac_system/d', projectAll: true })
...
- **表字段 vs 数据点**：`createAPI.query()` 只返回 Schema 定义的 fields，不返回 tags 值
