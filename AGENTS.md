...
## Project Overview
This is a TypeScript client library for Kesi (@kesi/client) built with Vite. The library provides API, authentication, form handling, and model management components.
...
The library includes a collection of built-in Kesi system models defined in `src/model/models.json`. These models are automatically registered in the `modelRegistry` and can be used by specifying the `name` prop on the `Model` component.
